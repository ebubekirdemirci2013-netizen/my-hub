...
-- ============================================================
--  Touch-Fly-Steuerung (iOS / iPad)
--  Ein Finger zieht auf dem Bildschirm → Charakter fliegt in
--  die entsprechende Richtung (horizontal + vertikal relativ
--  zur Kamera). Kein separater Auf/Ab-Button nötig.
-- ============================================================
...
local flyTouchStart   = Vector2.new(0, 0)
local flyTouchDelta   = Vector2.new(0, 0)
local flyTouchActive  = false

local function stopFlyTouchInput()
    for _, c in ipairs(flyTouchConns) do c:Disconnect() end
    flyTouchConns  = {}
    flyTouchId     = nil
    flyTouchActive = false
    flyTouchDelta  = Vector2.new(0, 0)
end

local function startFlyTouchInput()
    stopFlyTouchInput()
    if not UserInputService.TouchEnabled then return end

    table.insert(flyTouchConns, UserInputService.TouchStarted:Connect(function(input, processed)
        if processed then return end
        if flyTouchId ~= nil then return end
        flyTouchId     = input
        flyTouchActive = true
        flyTouchStart  = Vector2.new(input.Position.X, input.Position.Y)
        flyTouchDelta  = Vector2.new(0, 0)
    end))

    table.insert(flyTouchConns, UserInputService.TouchMoved:Connect(function(input, processed)
        if input ~= flyTouchId then return end
        flyTouchDelta = Vector2.new(input.Position.X, input.Position.Y) - flyTouchStart
    end))

    table.insert(flyTouchConns, UserInputService.TouchEnded:Connect(function(input, processed)
        if input ~= flyTouchId then return end
        flyTouchId     = nil
        flyTouchActive = false
        flyTouchDelta  = Vector2.new(0, 0)
...
local function startFly()
    ...
    flyConn = RunService.Heartbeat:Connect(function()
        ...
        if UserInputService.TouchEnabled then
            -- ── Touch (iOS / iPad): Drag-Geste steuert alle 3 Achsen ──
            if flyTouchActive and flyTouchDelta.Magnitude > FLY_TOUCH_DEADZONE then
                ...
                dir = flatR * dx + fwdDir * (-dy)
            else
                dir = Vector3.new(0, 0, 0)
            end
        else
            -- ── PC / WASD: MoveDirection + Tastatur-Vertikal ──
